                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="mb-10 mt-10">
                                        ...
                                        <div class="input-group mb-5">
                                            <input type="text" class="form-control" placeholder="Başlık" id="title_en_{{$setting->id}}" value="{{$setting->title_en}}" aria-label="Başlık"/>
                                            <span class="input-group-text">@</span>
                                            <input type="text" class="form-control" placeholder="Açıklama" id="description_en_{{$setting->id}}" value={{$setting->description_en}} aria-label="Açıklama"/>
                                            <span class="input-group-text">@</span>
                                            <div class="fv-row">
                                                <!--begin::Dropzone-->
                                                <div class="dropzone" id="dropzone_en_{{$setting->id}}">
                                                    <!--begin::Message-->
                                                    <div class="dz-message needsclick">
                                                        <!--begin::Icon-->
                                                        <i class="bi bi-file-earmark-arrow-up text-primary fs-3x"></i>
                                                        ...
                                                        <!--begin::Info-->
                                                        <div class="ms-4">
                                                            <input type="text" value="{{$setting->image_en}}" id="image_en_{{$setting->id}}" hidden>
                                                            <h3 class="fs-5 fw-bolder text-gray-900 mb-1">Resmi sürükle veya
                                                                yükle</h3>
                                                            <span
                                                                class="fs-7 fw-bold text-gray-400">Maksimum 1 dosya yükleyebilirsiniz</span>
...
@section('page_scripts')
    ...
    <script>
        ...
        var myDropzone_en_{{$setting->id}} = new Dropzone("#dropzone_en_{{$setting->id}}", {
            url: "{{route('admin.news.upload')}}", // Set the url for your upload script location
            paramName: "file", // The name that will be used to transfer the file
            maxFiles: 1,
            thumbnailWidth: "500",
            ...
            success: function (file, response) {
                $('#image_en_{{$setting->id}}').val(response.url);
                toastr.success('Görsel başarıyla yüklendi');
            },
            error: function (file, response) {
                toastr.error(response);
            ...
            removedfile: function (file) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: '{{route('admin.news.delete')}}',
                    data: {fileName: $('#image_en_{{$setting->id}}').val()},
                    dataType: 'json',
                    success: function (data) {
                        toastr.success('Görsel başarıyla silindi');
                    }
        ...
        $(document).ready(function () {
            $('#update').click(function () {
                ...
                data.push({
                    ...
                    title_en: $('#title_en_{{$setting->id}}').val(),
                    description_en: $('#description_en_{{$setting->id}}').val(),
                    image_en: $('#image_en_{{$setting->id}}').val()
                });
                @endforeach
                $.ajax({
                    headers: {
